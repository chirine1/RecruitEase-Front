  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        formData.append("title1", values.subTitle1);
        formData.append("paragraph1", values.paragraph1);
        formData.append("title2", values.subTitle2);
        formData.append("paragraph2", values.paragraph2);
        formData.append("img", logImg); // Append the image file

        // API call to submit the form data
        const response = await axios.post("/blog_post", formData, {
          headers: {
        ...
        resetForm();
        setLogoImg(null); // Reset the image file input
      } catch (error) {
